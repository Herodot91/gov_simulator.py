...
drill-downs, the public transit network (Metro/Tram rail, BRT/Bus/
Regional Rail road+regional transit) with named-street routes and
interchange markers, the Metropolitan Ring Road, Technopolis Expressway
and Regional Expressway, the Coach Terminal/Airport/Centrul Civic/CBD
zone map sites, each governance tier's own directorates/departments org
chart (plus interactive Prefecture Policies, two Prefecture Towns with
their own town councils and policies, a Current Policies review panel,
and a consolidated Directorates Dashboard), on-map district boundaries
once a municipality is selected, and CSV/JSON export. Vanilla CSS
throughout — no UI framework. See the [root README](../README.md) for
the full rationale behind each feature.
